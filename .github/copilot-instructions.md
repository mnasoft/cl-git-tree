## Инструкции для AI-агента: сборка и установка бинарника

- **Сборка бинарника**: для сборки используйте команду:
  ```bash
  sh ./build-bin.sh
  ```
  Если нет прав на запуск, выполните:
  ```bash
  chmod +x build-bin.sh
  sh ./build-bin.sh
  ```
- **Установка бинарника**: после сборки выполните:
  ```bash
  sh ./install-git-tree-bin.sh
  ```
  Это установит бинарник и вспомогательные файлы в bin/ или системный путь.

Проверяйте права на выполнение скриптов при необходимости.
