...
* [ ] ToDo
2. [ ] для команды git tree commit ключи использовать аналогичные git commit
3. [ ] сделать аналог команды git-timecapsule
     
* [ ] DoNe
1. [X] [[./src/commands/add.lisp]]
   - [X] cmd-add -- сделать разбор аргументов при помощи
     cl-git-tree/shell-utils:split-args-by-keys для ключа :EXCLUDE.
   - [X] cmd-add -- обновить помощь.
   - [X] откорректировать сигнатуру вызова для метода repo-add
     [[./src/loc/methods/repo-add.lisp]] чтоб гармонизировать ее с
     командой add.
2. [X] Исключить команду all [[./src/commands/all.lisp]]
3. [X] Реализовать новую структуру приекта
      cl-git-tree/
