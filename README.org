...
#+begin_src bash
git tree transport export               # краткий вывод: архивы в :url-xz
git tree transport export --verbose     # подробный вывод с причинами пропусков
git tree transport export --days 7      # ограничить по свежести коммита
git tree transport clean                # удалить *.tar.xz из :url-xz всех провайдеров
#+end_src

Импорт архивов на другой машине (архивы уже лежат в соответствующих :url-xz):
#+begin_src bash
git tree transport import               # распаковать *.tar.xz в bare-хранилища :url-git
#+end_src

*** Логика работы

1. *Export*: для каждого зарегистрированного провайдера с `:url-xz` создаёт tar.xz архивы из чистых репозиториев (без незакоммиченных изменений).
   - Фильтр по времени коммита: по умолчанию архивируются репозитории, обновлённые не позднее 30 дней назад.
   - Архив содержит только голый git-репозиторий (=git clone --bare=) без рабочих файлов.
   - Режим `--verbose` показывает причины пропусков (старые коммиты, незакоммиченные изменения и т.д.).

2. *Import*: обходит все локации с `:url-xz` и распаковывает найденные архивы в bare-хранилища, указанные в `:url-git`.
   - Существующее хранилище заменяется новым (удаляется и кладётся свежий архив).

3. *Clean*: удаляет все `*.tar.xz` архивы из каталогов `:url-xz` всех провайдеров.
   - Полезна для очистки после импорта на целевой машине.
