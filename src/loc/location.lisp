...
для вывода пользователю и в справочной информации.")
   (url-git
    ...
    :documentation
    "Базовый git-URL для группы репозиториев. Обычно оканчивается на '/'.
Используется для построения полного пути к конкретному репозиторию.")
   (url-xz
    :initarg :url-xz
    :initform nil
...
репозиториев в виде архивов.")
   (tar
    ...
    :documentation
    "Путь к tar-архиву, если используется для распространения.")
   (local
    :initarg :local
    :initform nil
...
- `local` — политика локального использования.
...
(defun add-location (id &key description url-git url-xz tar)
...
Возвращает созданный объект класса <location>."
  (when (gethash id *locations*)
    (warn "Перезаписываю существующую локацию с ключом ~A" id))
  (setf (gethash id *locations*)
